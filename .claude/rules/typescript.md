...
File naming exceptions (unicorn/filename-case):

- Route parameter files starting with `$` are allowed (e.g., `$id.tsx`, `$postId.tsx`)
...
From `packages/typescript-config/base.json`:
...
```ts
import { type User, getUser } from '@/lib/user'; // Good
export { type User, getUser }; // Good
import type { User } from '@/lib/user'; // Bad
...
- Group imports: external → internal (`@/` or `@oakoss/`) → relative
- Imports must be at top of file, with blank line after
- No duplicate imports (prefer inline: `import { A, B } from 'x'`)
- **No relative parent imports** - always use `@/` alias or package name
